...
void sigchild_handler(int sig)
{
  ...
  // waitpid : 프로세스 id 가 pid 인 프로세스의 상태 정보를 얻어오는데 주로 사용된다.
  // 에러라면 -1 , WNOHANG 이 사용되고 어떤 자식도 이용되지 않았다면 0 , 그 이외의 정상적인 경우 자식 프로세스의 PID
...
// 동적인 파일을 받았을 때 fork 함수로 자식 프로세스를 만든 후에 거기서 CGI 프로그램을 실행한다.
void serve_dynamic(int fd , char * filename , char *cgiargs, char * method)
{
...
// 폴더 안에서 특정 이름을 찾아서 파일이 동적인 건지 정적인 건지 알려주는 함수
int parse_uri(char * uri , char * filename , char *cgiargs) {
  char * ptr;
...
void doit(int fd) // HTTP 트랜 잭션을 처리한다.
{
  ...
  if(strcasecmp(method , "GET") || strcasecmp(method , "HEAD") == 0) { // strcasecmp 함수 : 대소문자를 구분하지 않고 문자열을 비교 한다.
  ...
  if(is_static) { // 정적 컨텐츠 라면
    ...
    serve_static(fd , filename , sbuf.st_size , method); // 정적 콘텐츠를 클라이언트에게 제공한다.
  }
  else
  { // 동적 콘텐츠 라면
    ...
    serve_dynamic(fd , filename , cgiargs, method); // 동적 컨텐츠를 제공한다.
  }
}

// 소켓은 네트워크 상의 다른 프로세스와 통신하기 위해 사용되는 파일
// 입력 ./tiny 5000  , argc = 2 , argv[0] = tiny , argv[1] = 5000
int main(int argc, char **argv) {
  ...
  listenfd = Open_listenfd(argv[1]); // Open_listenfd 함수를 호출해서 listen socket을 오픈한다. 인자로 포트번호를 넘겨준다.
  // 무한 서버 루프
  while (1) {
